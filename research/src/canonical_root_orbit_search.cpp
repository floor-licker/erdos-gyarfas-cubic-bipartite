...
int main(int argc, char** argv) {
    int first_orbit=1, last_orbit=3;
    ...
    if (argc==2) {
        if (!unreduced_root) {
            ...
            if (requested<1 || requested>3) {
                std::fprintf(stderr,
                    "usage: %s [orbit: 1|2|3 | --unreduced-root]\n",argv[0]);
                return 64;
            }
    ...
    discovery::Counts total;
    if (unreduced_root) {
        discovery::Branch branch;
    ...
    std::printf("TOTAL V=%d states=%llu attempted=%llu pair=%llu c8=%llu c16=%llu completions=%llu seconds=%.6f\n",
      discovery::V,(unsigned long long)total.states,(unsigned long long)total.attempted,
