...
static_assert(kSide >= 7 && kSide <= 29, "SIDE must be between 7 and 29");

constexpr std::array<char, 8> kMagic{{'E', 'G', '5', '8', 'C', 'E', 'R', '1'}};
...
class CertificateGenerator {
  public:
    ...
    Counts run(int orbit) {
        ...
        if (orbit == 2) {
            if (kSide < 8) {
                throw std::runtime_error("root orbit 2 requires SIDE >= 8");
            }
            first = {1, 3, 7};
            introduced_ = 8;
        } else if (orbit == 3) {
            if (kSide < 9) {
                throw std::runtime_error("root orbit 3 requires SIDE >= 9");
            }
            first = {1, 7, 8};
            introduced_ = 9;
        } else if (orbit != 1) {
            throw std::runtime_error("invalid root orbit");
        }

        add(first);
        recurse(1, first.b, first.c);
        return counts_;
    }
  ...
  private:
    ...
    bool path14_dfs(int current, int target, int used_blocks,
                    std::uint32_t seen_points, std::uint32_t seen_blocks,
        ...
        for (int block_id : incident_[current]) {
            const std::uint32_t block_bit = std::uint32_t{1} << block_id;
            ...
            for (int next : next_points) {
                ...
                const std::uint32_t point_bit = std::uint32_t{1} << next;
        ...
        for (int i = 0; i < 3; ++i) {
            for (int j = i + 1; j < 3; ++j, ++code) {
                ...
                if (path14_dfs(start, target, 0,
                               std::uint32_t{1} << start, 0U, witness)) {
    ...
    void recurse(int point, int last_q, int last_r) {
        ...
        if (point >= introduced_ ||
            static_cast<int>(blocks_.size()) >= kSide) {
            return;
        }
    ...
    try {
        if (argc != 3) {
            throw std::runtime_error(
                "usage: generate_eg_certificate OUTPUT "
                "(c8-only|1|2|3)");
        }
