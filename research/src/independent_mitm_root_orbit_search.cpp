...
struct Branch {
 std::vector<Line> lines; std::array<std::vector<int>,V> through; std::array<int,V> degree{}; std::array<std::bitset<V>,V> together{}; int introduced=7; Counts count; transcript_audit::SearchTranscript transcript;
 ...
 void recurse(int p,int lastq,int lastr){
  ...
  if(p==V){
   if(int(lines.size())==V){count.completions++;transcript.terminal(0);}
   else transcript.terminal(1);
   transcript.leave();return;
  }
  if(p>=introduced){transcript.terminal(2);transcript.leave();return;}
 ...
 Counts run(int orbit){
  ...
  insert({0,1,2});insert({0,3,4});insert({0,5,6});Line first{};
  if(orbit==1)first={1,3,5};
  else if(orbit==2){if(V<8){transcript.terminal(4);transcript.leave();return count;}first={1,3,7};introduced=8;}
  else{if(V<9){transcript.terminal(4);transcript.leave();return count;}first={1,7,8};introduced=9;}
  insert(first);recurse(1,first[1],first[2]);return count;
 }
 Counts run_unreduced_root(){transcript.begin(static_cast<std::uint8_t>(V),0);insert({0,1,2});insert({0,3,4});insert({0,5,6});recurse(1,-1,-1);return count;}
 bool transcript_matches_counts()const{return transcript.states()==count.states&&transcript.candidates()==count.attempted;}
};
}
int main(int argc,char**argv){int first=1,last=3;const bool unreduced_root=argc==2&&std::strcmp(argv[1],"--unreduced-root")==0;if(argc==2){if(!unreduced_root){int requested=std::atoi(argv[1]);if(requested<1||requested>3){std::fprintf(stderr,"usage: %s [orbit: 1|2|3 | --unreduced-root]\n",argv[0]);return 64;}first=last=requested;}}else if(argc!=1){std::fprintf(stderr,"usage: %s [orbit: 1|2|3 | --unreduced-root]\n",argv[0]);return 64;}auto start=std::chrono::steady_clock::now();mitm_verifier::Counts total;if(unreduced_root){mitm_verifier::Branch b;total=b.run_unreduced_root();if(!b.transcript_matches_counts()){std::fprintf(stderr,"transcript counter mismatch\n");return 70;}std::printf("UNREDUCED_ROOT states=%llu attempted=%llu pair=%llu c8=%llu c16=%llu completions=%llu\n",(unsigned long long)total.states,(unsigned long long)total.attempted,(unsigned long long)total.pair,(unsigned long long)total.c8,(unsigned long long)total.c16,(unsigned long long)total.completions);std::printf("TRANSCRIPT orbit=0 states=%llu candidates=%llu sha256=%s\n",(unsigned long long)b.transcript.states(),(unsigned long long)b.transcript.candidates(),b.transcript.hex_digest().c_str());std::fflush(stdout);}else for(int o=first;o<=last;o++){mitm_verifier::Branch b;auto c=b.run(o);if(!b.transcript_matches_counts()){std::fprintf(stderr,"transcript counter mismatch in orbit %d\n",o);return 70;}total+=c;std::printf("ORBIT %d states=%llu attempted=%llu pair=%llu c8=%llu c16=%llu completions=%llu\n",o,(unsigned long long)c.states,(unsigned long long)c.attempted,(unsigned long long)c.pair,(unsigned long long)c.c8,(unsigned long long)c.c16,(unsigned long long)c.completions);std::printf("TRANSCRIPT orbit=%d states=%llu candidates=%llu sha256=%s\n",o,(unsigned long long)b.transcript.states(),(unsigned long long)b.transcript.candidates(),b.transcript.hex_digest().c_str());std::fflush(stdout);}double sec=std::chrono::duration<double>(std::chrono::steady_clock::now()-start).count();std::printf("TOTAL V=%d states=%llu attempted=%llu pair=%llu c8=%llu c16=%llu completions=%llu seconds=%.6f\n",mitm_verifier::V,(unsigned long long)total.states,(unsigned long long)total.attempted,(unsigned long long)total.pair,(unsigned long long)total.c8,(unsigned long long)total.c16,(unsigned long long)total.completions,sec);return total.completions?2:0;}
