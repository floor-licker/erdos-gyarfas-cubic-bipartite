...
Header read_header(std::istream& input) {
    ...
    if (header.orbit < 0 || header.orbit > 3) {
        fail("invalid root orbit");
    }
    if (header.orbit == 0) {
        if (header.flags != kFlagC8) {
            fail("unreduced-root certificate has invalid flags");
        }
    } else if (header.flags != (kFlagC8 | kFlagC16)) {
        fail("root-orbit certificate has invalid flags");
    }
    return header;
}
...
class CertificateChecker {
  public:
    ...
    Counts check() {
        ...
        if (orbit_ == 2) {
            if (side_ < 8) {
                fail("root orbit 2 is unavailable at this side size");
            }
            first = {{1, 3, 7}};
            introduced_ = 8;
        } else if (orbit_ == 3) {
            if (side_ < 9) {
                fail("root orbit 3 is unavailable at this side size");
            }
            first = {{1, 7, 8}};
            introduced_ = 9;
        } else if (orbit_ != 1) {
        ...
        install(first);
        visit(1, first.point[1], first.point[2]);
        return counts_;
    }
  ...
  private:
    ...
    void visit(int point, int last_q, int last_r) {
        ...
        if (point >= introduced_ ||
            static_cast<int>(blocks_.size()) >= side_) {
            return;
        }
...
int main(int argc, char** argv) {
    try {
        if (argc != 2) {
            fail("usage: verify_eg_certificate CERTFILE|-");
        }
