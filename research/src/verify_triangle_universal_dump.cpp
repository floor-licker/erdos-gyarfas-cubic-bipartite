...
constexpr int kMaximumSide = 29;
constexpr std::array<char, 8> kMagic{{'E', 'G', '5', '8', 'T', 'R', 'I', '1'}};
...
class CertificateChecker {
  public:
    CertificateChecker(std::istream& input, const Header& header,
                       std::ostream* kernel_dump = nullptr)
        : input_(input), side_(header.side), orbit_(header.orbit),
          flags_(header.flags), kernel_dump_(kernel_dump) {}
  ...
  private:
    ...
    int introduced_ = 6;
    Counts counts_;
    std::ostream* kernel_dump_ = nullptr;
    ...
    void visit(int point, int last_q, int last_r) {
        ...
        if (kernel_dump_ != nullptr && blocks_.size() == 19U) {
            *kernel_dump_ << orbit_ << ' ' << kernel_index_++ << ' '
                          << introduced_ << ' ' << point << ' ';
            for (const Block& block : blocks_) {
                *kernel_dump_ << block.point[0] << ',' << block.point[1]
                              << ',' << block.point[2] << ';';
            }
            *kernel_dump_ << '\n';
        }

        // If every introduced point is already cubic, the connected
        // partial structure is a completed configuration on exactly
        // introduced_ points.  Detect it even when introduced_ < side_,
        // so one cap-side certificate excludes every smaller side size.
...
int main(int argc, char** argv) {
    try {
        if (argc != 2 && argc != 3) {
            fail("usage: verify_triangle_universal_certificate CERTFILE|- [KERNEL_DUMP]");
        }
        ...
        std::ostream* kernel_dump = nullptr;
        if (argc == 3) {
            kernel_file.open(argv[2], std::ios::out | std::ios::trunc);
            if (!kernel_file) {
                fail("cannot open kernel dump");
            }
            kernel_dump = &kernel_file;
        }

        const Header header = read_header(*input);
        CertificateChecker checker(*input, header, kernel_dump);
        ...
        std::cout << "VERIFIED EG58TRI1-UNIVERSAL"
                  << " side=" << header.side
                  << " orbit=" << header.orbit << '\n';
        print_counts(actual);
        return 0;
